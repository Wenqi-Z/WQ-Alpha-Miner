...
class _Program(object):
    ...
    def __init__(
        ...
        self.function_set = [func for func in function_set if func.arity != 0]
        self.fixed_function_set = [func for func in function_set if func.arity == 0]
    ...
    def build_program(self, random_state):
        ...
        if function.is_ts:
            func = copy(function)
            func.set_window(random_state)
            program = [func]
            terminal_stack = [func.arity]
        else:
            program = [function]
        ...
        while terminal_stack:
            ...
            choice = self.n_features + len(self.function_set) + len(self.fixed_function_set)
            ...
            if (depth < max_depth) and (method == "full" or choice <= len(self.function_set)):
                ...
                if self.const_range is not None:
                    terminal = random_state.randint(
                        self.n_features + 1 + len(self.fixed_function_set)
                    )
                else:
                ...
                if terminal == self.n_features + len(self.fixed_function_set):
                    ...
                    terminal = copy(self.fixed_function_set[terminal - self.n_features])
                    if terminal.is_ts:
                        terminal.set_window(random_state)
        ...
        for i, node in enumerate(self.program):
            if isinstance(node, _Function):
                if node.arity != 0:
                    ...
                    output += node.name + "(%s" % ", ".join(node.parmas_need)
                    if node.is_ts:
                        output += f", {node.n})"
                    else:
                        output += ")"
                    ...
                    while terminals[-1] == 0:
                        ...
                        terminals[-1] -= 1
                        node_ = stack.pop()
                        if node_ and node_.is_ts:
                            output += f", {node_.n})"
                        elif node_ and node_.group_arg:
                            output += f", {node_.group_arg})"
                        else:
                            output += ")"
    ...
    # Not adapted for factor expresion
        ...
        for node in self.program:
            if isinstance(node, _Function):
                if node.arity != 0:
                    ...
                    args_ = [X[:, self.feature_names.index(name)] for name in node.parmas_need]
                    t = node(*args_)
                    if apply_stack:
                ...
                if apply_stack:
                    ...
                    return np.repeat(node, X.shape[0]) if isinstance(node, float) else X[:, node]
    ...
    def get_all_indices(self, n_samples=None, max_samples=None, random_state=None):
        ...
        if self._indices_state is None and random_state is None:
            raise ValueError(
                "The program has not been evaluated for fitness yet, indices not available."
            )
    ...
    def raw_fitness(self, data, args):
        ...
        return self.metric(data, expression, args)
        ...
        for node in mutate:
            ...
            if not mutated:
                ...
                if terminal == self.n_features + len(self.fixed_function_set):
                    ...
                    replacement = copy(self.fixed_function_set[terminal - self.n_features])
                    replacement.set_window(random_state)
                    program[node] = replacement
                else:
